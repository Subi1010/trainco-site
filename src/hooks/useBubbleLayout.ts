...
import { resolveCollisions, type LayoutRect } from "@/utils/resolveCollisions";
...
function computeZigzag(
  ...
  while (i < count) {
    ...
    if (i + 1 < count) {
      ...
      if (rx >= leftRightEdge) {
        rects[i + 1] = { x: rx, y: y + pairYShift, w: widths[i + 1], h: heights[i + 1] };
        y += bubbleH + pairYShift + rowGap;
        i += 2;
      } else {
        y += bubbleH + rowGap;
        rects[i + 1] = { x: containerW - widths[i + 1] - rMargin, y, w: widths[i + 1], h: heights[i + 1] };
        y += bubbleH + rowGap;
        i += 2;
      }
    } else {
...
export function useBubbleLayout({
  ...
  const setBubbleRef = useCallback((label: string, el: HTMLElement | null) => {
  ...
  useLayoutEffect(() => {
    ...
    if (!container || labels.length === 0) { setResolved(null); return; }
    ...
    const widths = labels.map((l) => bubbleEls.current.get(l)?.offsetWidth ?? 140);
    const heights = labels.map((l) => bubbleEls.current.get(l)?.offsetHeight ?? 48);

    const rects = computeZigzag(labels, widths, heights, cw, maxY, padding);
  ...
  return { containerRef, setBubbleRef, positions: resolved ?? fallback, ready: resolved !== null };
}
